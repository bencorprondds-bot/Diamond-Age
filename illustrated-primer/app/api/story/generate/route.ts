...
export async function POST(request: Request) {
  try {
    const { heroName, lovesToDo, wantsToLearn, specialTrait, hobbies, previousStory, userContribution } = await request.json();
...
Continue the story (about 100-150 words) building on what Harlow wrote. Make her contribution meaningful and exciting. Keep the tone engaging and age-appropriate. End with another opportunity for her to respond. The hero's name is ${heroName}.`;
    } else {
